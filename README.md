...
> **MedTrust** 是一套基于 **区块链防篡改存证**、**动态风险评估引擎**、**患者自主知情授权** 与 **大模型 AI 辅助诊疗** 的跨医疗机构电子病历安全共享与可信协同系统。
...
│ (9张核心表: 医生/患者/ │       │  区块链存证与验证账本  │        │ (FastAPI + LangChain) │
│  电子病历/授权/审计等)│       │  (Merkle Root + TxID) │        │     [Port: 8000]      │
│     [Port: 3306]      │       │                       │        └───────────────────────┘
└───────────────────────┘       └───────────────────────┘
            ▲
            │ 攻防演练直连注入
